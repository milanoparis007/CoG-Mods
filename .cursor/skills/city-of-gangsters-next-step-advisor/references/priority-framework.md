...
1. Live pain
   Favor tasks backed by repeated warnings, failed patches, disabled features, or obvious runtime oddities in `Player.log`.
2. Recency pressure
...
4. `Player.log` currently shows `HostileMobileSelectionFixPatch` failing on `Microsoft.CSharp` load, repeated `GangOps.Pact ... skipped=no-eligible-gangs`, `MobilePickUtil` type drift, and repeated `BossRoleSetter` warnings.
