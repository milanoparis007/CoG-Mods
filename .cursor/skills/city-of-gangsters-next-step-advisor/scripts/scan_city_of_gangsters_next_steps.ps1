...
if (-not $RepoRoot)
{
	# Default: repository root (four levels above this script under .cursor/skills/<skill>/scripts/)
...
try
{
	...
	else
	{
		$logOutput = @(Invoke-Capture {
			rg -n "\[Warning|\[Error|AccessTools.TypeByName|failed|disabled|skipped=no-eligible-gangs|Microsoft\.CSharp|Outpost disappeared|BossRoleSetter|Multi-crew vehicle|CopWarSystem initialized|gangOpsMode|activeSubsystems" Player.log -S
		})
